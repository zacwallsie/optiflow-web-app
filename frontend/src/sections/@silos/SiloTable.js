...
export default function SiloTable() {
	...
	const confirmDelete = () => {
		// Extract Silo Ids from selected
		axios
	...
	const handleNameClick = (siloId) => {
		navigate(`/flow/silos/${siloId}`)
	}
	...
	const visibleRows = React.useMemo(() => {
		return stableSort(schemas, getComparator(order, orderBy))
			.filter((row) => row.silo_name.toLowerCase().includes(searchTerm.toLowerCase()))
